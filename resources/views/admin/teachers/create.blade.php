...
<style>
    ...
    .preview-pic { width:140px;height:140px;border-radius:50%;object-fit:cover;border:3px solid #e5e9f0; }
</style>
@endpush
...
<div class="d-flex align-items-center gap-3 mb-4">
    <a href="{{ route('admin.teachers.index') }}" class="btn btn-sm btn-outline-secondary"><i class="bi bi-arrow-left"></i></a>
    <div>
        <h2 class="fw-bold mb-0" style="font-size:1.4rem;color:#1e3a5f"><i class="bi bi-person-plus-fill me-2"></i>Register Teacher</h2>
        <p class="text-muted small mb-0">Create a new teacher account and profile.</p>
    </div>
</div>

<form method="POST" action="{{ route('admin.teachers.store') }}" enctype="multipart/form-data" novalidate>
@csrf
...
<div class="row g-4">
    ...
    <div class="col-lg-8">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}"
                               ...
                               placeholder="e.g. Shams ur Rehman" required />
        ...
        <div class="card qams-card mb-4">
            <div class="card-body p-4">
                <p class="section-title"><i class="bi bi-briefcase me-2"></i>Professional Profile</p>
                <div class="row g-3">
                    <div class="col-md-12">
                        <label class="form-label fw-semibold">Education</label>
                        <textarea name="education" rows="3"
                                  class="form-control @error('education') is-invalid @enderror"
                                  placeholder="Highest qualification, institution, year…">{{ old('education') }}</textarea>
                        @error('education')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-12">
                        <label class="form-label fw-semibold">Job History</label>
                        <textarea name="job_history" rows="4"
                                  class="form-control @error('job_history') is-invalid @enderror"
                                  placeholder="Previous positions, schools, years of experience…">{{ old('job_history') }}</textarea>
                        @error('job_history')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card qams-card">
            <div class="card-body p-4">
                <p class="section-title"><i class="bi bi-book-fill me-2"></i>Assign Subjects</p>
                @if($subjects->isEmpty())
                    <p class="text-muted small">No subjects available. <a href="{{ route('admin.subjects.create') }}">Create subjects first.</a></p>
                @else
                ...
                <div class="d-flex justify-content-center mb-3">
                    <label for="picture" style="cursor:pointer">
                        <div class="picture-box" id="picture-box">
                            <i class="bi bi-person-badge" style="font-size:2.5rem;color:#cbd5e1"></i>
                            <span>Click to upload</span>
                        </div>
                        <img id="pic-preview" src="" alt="" class="preview-pic" style="display:none" />
                    </label>
                </div>
